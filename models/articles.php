...
class Article
{
    ...
    // Metoda, ki vstavi novico v bazo v tabelo articles
    public static function create($title, $abstract, $text){
        ...
        $query = "INSERT INTO articles (title, abstract, text, date, user_id) VALUES ('$title', '$abstract', '$text', '$date', '$user_id');";
        if($db->query($query)){
            return true;
        }
        else{
    ...
    // Metoda, ki iz baze vrne vse novice nekega uporabnika z id-jem
    public static function userArticles($user_id)
    {
        $db = Db::getInstance(); // pridobimo instanco baze
